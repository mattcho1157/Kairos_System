...
if (isset($_POST['submitlogin'])) {
	...
	if (!$userData) {
		...
		if ($pwd != $userData['pwd']) {
			...
			if ($userData['activated']) {
				//if 2FA successful - account activated
				...
				alert('danger', 'PLEASE ACTIVATE YOUR ACCOUNT VIA EMAIL TO LOGIN');
			}
		}
	}
	...
	$userbaseData = DB::queryFirstRow('select * from userbase where username = %s', $username);

	if ($usersData) {
		//username already exists
		alert('danger', 'USERNAME ALREADY REGISTERED');
	} elseif (!$userbaseData) {
		//username is not a valid student s-number
		//in the real application, this is where mentor registration will occur
		alert('danger', 'YOU ARE POSSIBLY A MENTOR OR AN ADMIN');
	} else {
		//new user
		if ($pwd == $confirmpwd) {
			//passwords match
			$email = $userbaseData['email'];
			$fname = $userbaseData['fname'];
			$lname = $userbaseData['lname'];

			//generating token for account activation
